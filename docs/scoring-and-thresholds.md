...
When the floor is triggered, the raw scores are restored on the candidates so
the API consumer sees meaningful probability values instead of all zeros.
...
| `1.0`           | Only accept when the top candidate has a perfect normalised score (i.e. it is the single candidate above the floor) |
